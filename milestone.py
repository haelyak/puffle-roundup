...
def make_fence2(starting_position):
    ...
    slat2 = box(size = vec(.1, .5, 5), axis = vec(1,0,0), pos = vec(3,1.7,-3), color = .35*vec(1.0, 0.7, 0.3))
...
def make_fence3(starting_position):
    ...
    slat3 = box(size = vec(.1, .5, 5), axis = vec(1,0,0), pos = vec(-3,1.7,-3), color = .35*vec(1.0, 0.7, 0.3))
...
snow = []
for i in range(28):
    snow += make_snow(starting_position = 9*vec.random())
...
def snowfall(snow):
    ...
    if snow.pos.y < 0:          
        ...
        snow.pos.x = 9*random()*choice([1,-1])
        snow.pos.z = 9*random()*choice([1,-1])
